...
class PiaRealtime extends EventEmitter {
  ...
  connect(): void {
    ...
    this.socket.onopen = () =>
      this.socket.send(JSON.stringify({ action: 'auth', api_key: this.apiKey }))
  ...
  subscribe(symbols: string | string[]): void {
    this.subscriptions = [
      ...new Set([...this.subscriptions, ...(Array.isArray(symbols) ? symbols : [symbols])])
    ]
    if (this.state === 'AUTHENTICATED')
      this.sendSubscribe(Array.isArray(symbols) ? symbols : [symbols])
  }
  unsubscribe(symbol: string): void {
    this.subscriptions = this.subscriptions.filter((item) => item !== symbol)
    if (this.state === 'AUTHENTICATED')
      this.socket?.send(JSON.stringify({ action: 'unsubscribe', symbols: [symbol] }))
  }
...
class ApiResource {
  ...
  protected get(path: string, params?: Json): Promise<any> {
    return this.api.request(path, 'GET', undefined, params)
  }
  protected post(path: string, body: Json): Promise<any> {
    return this.api.request(path, 'POST', body)
  }
}
...
class MarketApi extends ApiResource {
  ...
  getRealizedVolatility = (symbol?: string) =>
    this.get('/api/v1/market/realized-volatility', symbol ? { symbol } : undefined)
  getImpliedVolatility = (symbol?: string) =>
    this.get('/api/v1/market/implied-volatility', symbol ? { symbol } : undefined)
  getCandles = (symbol: string, params?: Json) =>
    this.get(`/api/v1/market/history/${encodeURIComponent(symbol)}`, {
      resolution: params?.timeframe || params?.resolution || '1m',
      limit: params?.limit || 1000,
      before: params?.before
    })
  getOrderBook = (symbol: string) =>
    this.get(`/api/v1/market/orderbook/${encodeURIComponent(symbol)}`)
}
...
class MacroApi extends ApiResource {
  ...
  getCentralBankStance = (bank: string) =>
    this.get(`/api/v1/central-banks/${encodeURIComponent(bank)}/stance`)
}
...
class FixedIncomeApi extends ApiResource {
  ...
  getHistory = (tenor: string) =>
    this.get(`/api/v1/rates/history/${encodeURIComponent(tenor)}`, { country: 'US' })
}
...
class IntelligenceApi extends ApiResource {
  ...
  getInsights = (symbol: string) =>
    this.get(`/api/v1/market/insights/${encodeURIComponent(symbol)}`)
}
...
export class PiaApiClient {
  ...
  constructor(private readonly config: PiaApiOptions) {
    ...
    this.economic = new EconomicApi(this)
    this.news = new NewsApi(this)
    this.social = new SocialApi(this)
    this.options = new OptionsApi(this)
    this.macro = new MacroApi(this)
    this.fixedIncome = new FixedIncomeApi(this)
    this.geosignals = new GeoApi(this)
    this.energy = new EnergyApi(this)
    this.sec = new SecApi(this)
  ...
  async request(path: string, method: string, body?: Json, params?: Json): Promise<any> {
    ...
    if (!response.ok) {
      ...
      ) as Error & { statusCode?: number; endpoint?: string; code?: string }
